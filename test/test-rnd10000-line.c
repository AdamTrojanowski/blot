...
int main(void)
{
	...
	static gint64 datax[DATA_COUNT];
	static gint64 datay[DATA_COUNT];

	for (int i=0; i<DATA_COUNT; i++) {
		datax[i] = rand()%DATA_X_MAX;
	}

	for (int i=0; i<DATA_COUNT; i++) {
		datay[i] = rand()%DATA_Y_MAX;
	}
	...
	blot_figure_line(fig, BLOT_DATA_INT64,
			    DATA_COUNT, datax, datay,
			    9, "scatter", &error);
	FATAL_ERROR(error);
